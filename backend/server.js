...
app.post(
  "/api/uploads",
  checkJwt,
  checkScope(["create:files", "create:folders"]),
  upload.array("files"),
  async (req, res) => {
    const {
      folderId,
    ...
    try {
      for (const file of files) {
        // Keep spaces in the original filename for database storage
        const originalFileName = file.originalname;

        // For S3 key, replace spaces with underscores
        const s3FileName = file.originalname.replace(/ /g, "_");

        const fileKey = folderId
          ? `${folderName.replace(/ /g, "_")}/${Date.now()}_${s3FileName}`
          : `${Date.now()}_${s3FileName}`;
        ...
        // Store original filename with spaces in database
        ...
        const values = folderId
          ? [
              ...
              country,
              category,
              originalFileName, // Store original filename with spaces
              fileUrl,
              fileKey, // S3 key with underscores
              isPublic === "true",
              workpackage || null,
              isMeeting === "true",
              isDeliverable === "true",
